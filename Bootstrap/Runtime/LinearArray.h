...
constexpr static auto iotaLinearArray(T const offset = {}) {

    static_assert(N >= T {}, "Negative sizes not allowed in iota_LinearArray()");

    return Detail::integerSequenceGenerateLinearArray<T>(offset, MakeIntegerSequence<T, N>());
}

using Detail::integerSequenceGenerateLinearArray;